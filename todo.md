...
- [x] テーマ切り替えボタンの共通コンポーネント化
  - ThemeToggleButtonコンポーネントの作成
  - header.tsx、floating-menu.tsxでの重複コード削除
  - TypeScript型定義とアクセシビリティ対応
  - カスタマイズ可能なProps（variant、size、className）の実装
  - 完了時間: 15分

- [x] 問い合わせボタンの共通コンポーネント化
  - ContactButtonコンポーネントの作成（フローティングメニュー専用）
  - floating-menu.tsxでの重複コード削除
  - 「お問い合わせ」ツールチップ付きアイコンボタンの実装
  - header.tsxは元のインライン実装に戻して分離
  - シンプルな設計でメンテナンス性向上
  - 完了時間: 30分
